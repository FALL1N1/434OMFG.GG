...
void MotionMaster::MoveChargeCalc(PathGenerator path, float speed, uint32 id, bool generatePath)
{
	if (Impl[MOTION_SLOT_CONTROLLED] && Impl[MOTION_SLOT_CONTROLLED]->GetMovementGeneratorType() != DISTRACT_MOTION_TYPE)
		return;

	// Mainaz: do not allow to move with UNIT_FLAG_DISABLE_MOVE
	...
	if (_owner->GetTypeId() == TYPEID_PLAYER)
	{
		...
		Mutate(new PointMovementGenerator<Player>(id, dest.x, dest.y, dest.z, generatePath, speed, path.GetUnitTarget()), MOTION_SLOT_CONTROLLED);
	}
	else
	{
		;//sLog->outDebug(LOG_FILTER_GENERAL, "Creature (Entry: %u GUID: %u) charge point (X: %f Y: %f Z: %f)",
		 //_owner->GetEntry(), _owner->GetGUIDLow(), x, y, z);
		Mutate(new PointMovementGenerator<Creature>(id, dest.x, dest.y, dest.z, generatePath, speed, path.GetUnitTarget()), MOTION_SLOT_CONTROLLED);
	}
}
...
void MotionMaster::MoveCharge(PathGenerator path, float speed, uint32 id, bool forCharge)
{
    G3D::Vector3 dest = path.GetActualEndPosition();

	if (forCharge)
	{
		MoveChargeCalc(path, speed, id);
	}
	else
	{

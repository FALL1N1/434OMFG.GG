...
protected:
    TargetedMovementGeneratorMedium(Unit* target, float offset, float angle, uint32 spellId = 0) :
        ...
        i_recalculateTravel(false), i_targetReached(false), i_spell(spellId), i_recalculateSpeed(false), i_targetStoped(false)
    {
    }
...
protected:
	...
	void _updateSpeed(T* owner, float &x, float &y, float &z);

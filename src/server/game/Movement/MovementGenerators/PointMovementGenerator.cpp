...
{
	if (id == EVENT_CHARGE && m_target)
	{
		bool target_moved = false;
		float threshold = 0.2f;
		float x = m_target->GetPositionX();
		float y = m_target->GetPositionY();

		if (x + threshold > i_x || x - threshold < i_x)
		{
			target_moved = true;
		}
		else if (y + threshold > i_y || y - threshold < i_y)
		{
			target_moved = true;
		}
		...
		if (target_moved)
		{
			PathGenerator new_path(unit);

			float objSize = m_target->GetObjectSize();
			float targetObjectSize = std::min(objSize, 4.0f);

			bool result = new_path.CalculatePath(x, y, m_target->GetPositionZ() + targetObjectSize, false, true);

			if (result)
			{
				new_path.ReducePathLenghtByDist(objSize); // move back

				G3D::Vector3 dest = new_path.GetActualEndPosition();

				Movement::MoveSplineInit init(unit);
				init.MovebyPath(new_path.GetPath());
				if (speed > 0.0f)
					init.SetVelocity(speed);
				init.Launch();
			}

...
m_respawnDelay(300), m_corpseDelay(60), m_respawnradius(0.0f), m_walkmode(0.0f), m_reactState(REACT_AGGRESSIVE), m_canRegen(true),
...
void Creature::Update(uint32 diff)
{
	...
	if (GetCharmerOrOwner())
	{
		...
		if ((m_idleTimer <= 0 && !IsIdle()) || m_idleTimer > SUMMON_IDLE_TIMER)
		{
			m_idleTimer = 0;
			SetIdle(true);
		}
...
void Creature::setDeathState(DeathState s)
{
    ...
    if (s == JUST_DIED)
    {
		...
		if (currentPlayers != -1)
		{
			...
			const float  percentagePerPlayer = 15.0f; // percentage of spawntime that's reduced per extra player after minPlayersForReduction is reached

...
namespace Trinity
{
    ...
    */
    ...
    class NearestPlayerToPlayerRangeCheck
    {
    public:
        NearestPlayerToPlayerRangeCheck(Player const* obj, float range) : i_obj(obj), i_range(range) { }

        bool operator()(Player* u)
        {
            if (u->isAlive() && i_obj->IsWithinDistInMap(u, i_range) && u != i_obj)
            {
                i_range = i_obj->GetDistance(u);
                return true;
            }
    ...
    private:
        ...
        NearestPlayerToPlayerRangeCheck(NearestPlayerInObjectRangeCheck const&);
    };

        ...
        case SPELLFAMILY_PALADIN:
            switch (m_spellInfo->Id)
            {
               case 20271: // Judgement
					...
					// T13 Retribution Paladin 2P 
					if (m_caster->HasAura(105765)) // T13 PALADIN
					{
						m_caster->CastSpell(m_caster, 105767, true); // Virtuous Empowerment
						return; // to prevent another charge
					}
                    break;
...
void Spell::EffectCharge(SpellEffIndex /*effIndex*/)
{
    ...
    if (effectHandleMode == SPELL_EFFECT_HANDLE_LAUNCH_TARGET)
    {
        if (m_preGeneratedPath.GetPathType() & PATHFIND_NOPATH)
        {
			...
			m_caster->GetMotionMaster()->MoveCharge(m_preGeneratedPath, SPEED_CHARGE, EVENT_CHARGE, true);
    }

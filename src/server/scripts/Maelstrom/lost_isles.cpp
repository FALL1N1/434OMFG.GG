...
/*
class npc_tornade_gob : public CreatureScript // Quest Warchief's Revenge
{
public:
	...
	struct npc_tornade_gobAI : public npc_escortAI
	{
		...
		void PassengerBoarded(Unit* who, int8 /*seatId*//*, bool apply)
		{
			if (apply)
			{
				...
				me->GetMotionMaster()->MovePath(36178, false);
			}
			else
				me->RemoveAura(68436);
		}
		...
		void UpdateEscortAI(const uint32 diff)
		{
			if (isBoarded)
			{
				if (isBoarded2)
				{
					if (_checkDespawn <= diff) // Based on timer
					{
						if (Vehicle* vehicle = me->GetVehicleKit())
							me->GetVehicleKit()->RemoveAllPassengers();
						me->ExitVehicle(); // Kick the player from the vehicle ^^
						me->DespawnOrUnsummon();
						_checkDespawn = 1000;
					}
					else
						...
						if (me->GetVehicleKit())
							if (Unit* u = me->GetVehicleKit()->GetPassenger(0))
								if (u->GetTypeId() == TYPEID_PLAYER)
								{
									...
									{
										isBoarded2 = true;
										_checkDespawn = 29045; // Closest I can get so far, perhaps reduce the timer a little, but for now it should be fine
										SetEscortPaused(true);
										me->GetMotionMaster()->MovePath(361780, false);
...
void AddSC_lost_isle()
{
    ...
    new npc_girocoptere();
    // new npc_tornade_gob(); // Using SAI instead

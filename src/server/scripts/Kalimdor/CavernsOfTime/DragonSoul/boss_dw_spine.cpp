        ...
        void UpdateAI(uint32 const diff)
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_SPAWN_BLOOD:
                {
                    ...
                    Trinity::Containers::RandomResizeList(Spawner, 1);
    ...
    class spell_searing_plasma_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            if (GetCaster()->GetMap() && GetCaster()->GetMap()->GetDifficulty() == RAID_DIFFICULTY_10MAN_NORMAL ||
                GetCaster()->GetMap() && GetCaster()->GetMap()->GetDifficulty() == RAID_DIFFICULTY_10MAN_HEROIC)
                Trinity::Containers::RandomResizeList(targets, 1);
...
public:
    ...
    class spell_blood_corruption_death_AuraScript : public AuraScript
    {
        ...
        {
            //aurEff->GetBase()->SetDuration(GetSpellInfo()->Effects[EFFECT_0].CalcValue());
        }
            ...
            if (Unit* owner = GetUnitOwner())
            {
                if (Creature* dwing = owner->FindNearestCreature(NPC_SPINE_OF_DEATHWING, 500.0f, true))
                {
                    ...
                    if (GetTargetApplication()->GetRemoveMode() == AURA_REMOVE_BY_ENEMY_SPELL)
                    {
                        ...
                        std::list<Player*> targets;
                        Trinity::AnyPlayerInObjectRangeCheck u_check(dwing, 500.0f);
                        Trinity::PlayerListSearcher<Trinity::AnyPlayerInObjectRangeCheck> searcher(dwing, targets, u_check);
                        dwing->VisitNearbyWorldObject(500.0f, searcher);

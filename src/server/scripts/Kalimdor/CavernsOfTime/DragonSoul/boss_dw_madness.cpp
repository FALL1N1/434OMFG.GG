...
public:
    ...
    struct between_maelstrom_platformsAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 const diff)
        {
            ...
            Trinity::AnyPlayerInObjectRangeCheck u_check(me, 15.0f);
...
public:
    ...
    struct boss_deathwingAI : public BossAI
    {
        ...
        {
            std::list<Player*> targets;
            Trinity::AnyPlayerInObjectRangeCheck u_check(me, 1000.0f);
            Trinity::PlayerListSearcher<Trinity::AnyPlayerInObjectRangeCheck> searcher(me, targets, u_check);
            me->VisitNearbyWorldObject(1000.0f, searcher);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case PHASE_2:
                    ...
                    me->PlayOneShotAnimKit(ANIM_KIT_FALL_FORWARD);
        ...
        void DamageTaken(Unit* who, uint32 &damage)
        {
            ...
            if (me->HealthAbovePct(1.5f) && (me->GetMaxHealth() *0.015f) > (me->GetHealth() - damage))
            {
                if (encounterEnd == false)
                {
                    ...
                    who->SummonGameObject(RAID_MODE(GO_ELEMENTIUM_FRAGMENT_10N, GO_ELEMENTIUM_FRAGMENT_25N, GO_ELEMENTIUM_FRAGMENT_10H, GO_ELEMENTIUM_FRAGMENT_25H), -12100.5f, 12173.6f, -2.73425f, 5.09636f, 0.0f, 0.0f, -0.559193f, 0.829038f, 100000);

                    Map::PlayerList const& PlayerList = instance->instance->GetPlayers();
                    AchievementEntry const* dw_mad1 = sAchievementStore.LookupEntry(6116);
                    AchievementEntry const* dw_mad2 = sAchievementStore.LookupEntry(6177);
                    AchievementEntry const* dw_mad3 = sAchievementStore.LookupEntry(6107);
                    for (Map::PlayerList::const_iterator itr = PlayerList.begin(); itr != PlayerList.end(); ++itr)
                    {
                        if (Player* player = itr->getSource())
                        {
                            if (IsHeroic())
                                player->CompletedAchievement(dw_mad1);
                            else
                            {
                                player->CompletedAchievement(dw_mad2);
                                player->CompletedAchievement(dw_mad3);
                            }
                        }
                    }
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        if (creature->GetInstanceScript())
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, TRALL_MENU, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
...
public:
    ...
    class spell_carrying_winds_SpellScript : public SpellScript
    {
        ...
        bool Load()
        {
            spell = GetSpellInfo()->Effects[EFFECT_0].CalcValue();
...
public:
    ...
    class spell_hemorrhage_SpellScript : public SpellScript
    {
        ...
        bool Load()
        {
            triggerSpell = GetSpellInfo()->Effects[EFFECT_0].CalcValue();
...
public:
    ...
    class spell_elementium_blast_SpellScript : public SpellScript
    {
        ...
        bool Load()
        {
            initialDamage = GetSpellInfo()->Effects[EFFECT_0].CalcValue();
            return GetCaster()->GetTypeId() == TYPEID_UNIT;
        }

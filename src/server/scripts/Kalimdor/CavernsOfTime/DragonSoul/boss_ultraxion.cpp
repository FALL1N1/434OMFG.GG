...
public:
    ...
    struct boss_ultraxionAI : public BossAI
    {
        ...
        void UpdateAI(uint32 const diff)
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_TWILIGHT_BURST:
                {
                    ...
                    Trinity::AnyPlayerInObjectRangeCheck u_check(me, 80.0f);
                    Trinity::PlayerListSearcher<Trinity::AnyPlayerInObjectRangeCheck> searcher(me, targets, u_check);
                    me->VisitNearbyWorldObject(80.0f, searcher);
        ...
        void JustDied(Unit* killer)
        {
            ...
            killer->SummonGameObject(RAID_MODE(GO_LESSER_CACHE_OF_THE_ASPECTS_10N, GO_LESSER_CACHE_OF_THE_ASPECTS_25N, GO_LESSER_CACHE_OF_THE_ASPECTS_10H, GO_LESSER_CACHE_OF_THE_ASPECTS_25H), -1753.63f, -2370.01f, 340.845f, 4.8f, 0.0f, 0.0f, -0.66913f, 0.743145f, 100000);
            _JustDied();
        }
...
public:
    ...
    {
        if (player->GetPrimaryTalentTree(player->GetActiveSpec()) == TALENT_TREE_PRIEST_HOLY ||
            player->GetPrimaryTalentTree(player->GetActiveSpec()) == TALENT_TREE_PRIEST_DISCIPLINE ||
            player->GetPrimaryTalentTree(player->GetActiveSpec()) == TALENT_TREE_PALADIN_HOLY ||
            player->GetPrimaryTalentTree(player->GetActiveSpec()) == TALENT_TREE_DRUID_RESTORATION ||
            player->GetPrimaryTalentTree(player->GetActiveSpec()) == TALENT_TREE_SHAMAN_RESTORATION)
            return false;
...
public:
    ...
    class spell_hour_of_twilight_kill_SpellScript : public SpellScript
    {
        ...
        {
            if (GetCaster()->GetMap() && GetCaster()->GetMap()->GetDifficulty() == RAID_DIFFICULTY_10MAN_NORMAL)
            {
            ...
            if (GetCaster()->GetMap() && GetCaster()->GetMap()->GetDifficulty() == RAID_DIFFICULTY_10MAN_HEROIC)
            {
            ...
            if (GetCaster()->GetMap() && GetCaster()->GetMap()->GetDifficulty() == RAID_DIFFICULTY_25MAN_HEROIC)
            {
    ...
    class spell_fading_light_dps_SpellScript : public SpellScript
    {
        ...
        void FilterTarget(std::list<WorldObject*>& targets)
        {
            ...
            if (Unit* caster = GetCaster())
            {
                ...
                {
                    if ((*iter)->GetPrimaryTalentTree((*iter)->GetActiveSpec()) != TALENT_TREE_PRIEST_HOLY &&
                        (*iter)->GetPrimaryTalentTree((*iter)->GetActiveSpec()) != TALENT_TREE_PRIEST_DISCIPLINE &&
                        (*iter)->GetPrimaryTalentTree((*iter)->GetActiveSpec()) != TALENT_TREE_PALADIN_HOLY &&
                        (*iter)->GetPrimaryTalentTree((*iter)->GetActiveSpec()) != TALENT_TREE_DRUID_RESTORATION &&
                        (*iter)->GetPrimaryTalentTree((*iter)->GetActiveSpec()) != TALENT_TREE_SHAMAN_RESTORATION &&
                        (*iter)->GetPrimaryTalentTree((*iter)->GetActiveSpec()) != TALENT_TREE_WARRIOR_PROTECTION &&
                        (*iter)->GetPrimaryTalentTree((*iter)->GetActiveSpec()) != TALENT_TREE_DEATH_KNIGHT_BLOOD &&
                        (*iter)->GetPrimaryTalentTree((*iter)->GetActiveSpec()) != TALENT_TREE_PALADIN_PROTECTION &&
                        (*iter)->GetPrimaryTalentTree((*iter)->GetActiveSpec()) != TALENT_TREE_DRUID_FERAL_COMBAT)
                        dpsTargets.push_back((*iter));
                }
                ...
                else if (GetCaster()->GetMap() && GetCaster()->GetMap()->GetDifficulty() == RAID_DIFFICULTY_10MAN_HEROIC)
                    Trinity::Containers::RandomResizeList(targets, 2);
                else if (GetCaster()->GetMap() && GetCaster()->GetMap()->GetDifficulty() == RAID_DIFFICULTY_25MAN_NORMAL)
...
public:
    ...
    class spell_timeloop_AuraScript : public AuraScript
    {
        ...
        bool Load()
        {
            absorbChance = GetSpellInfo()->Effects[EFFECT_1].CalcValue();
        ...
        void OnProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            if (Unit* owner = GetOwner()->ToPlayer())
            {
                if (owner->ToPlayer())
                {
                    ...
                    std::list<Player*> targets;
                    Trinity::AnyPlayerInObjectRangeCheck u_check(owner, 50.0f);
                    Trinity::PlayerListSearcher<Trinity::AnyPlayerInObjectRangeCheck> searcher(owner, targets, u_check);
                    owner->VisitNearbyWorldObject(50.0f, searcher);
